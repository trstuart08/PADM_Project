...
def find_highest_probability_proposition(propositions, bias_mode1 = False):
    ...
    Returns
    -------
    A single Proposition class 
    
        If one of the propositions in the propositions list has the uniquely maximum associated probability, then that proposition is returned. If multiple propositions have the same probability, then one is randomly selected if bias_mode1 is False.
        
        Otherwise, if bias_mode1 is True and if multiple propositions share a maximum probability value, and at least one has a mode = 1, value, then a maximum probability proposition with a mode of 1 is randomly returned. Otherwise, if multiple propositions share the same probability and none are assigned a mode of 1, then a random proposition is selected.
    ...
    if len(max_indices) == 1:
        ...
        if bias_mode1 == False:
            return random.choice(poss_props)
        ...
        # Give precedence to returning an assignment of mode 1
            ...
            if len(mode1_indices) == 1:
                ...
                return random.choice(prop_choices)
            else:
                return random.choice(poss_props)
...
def logical_test(clause, can_props, debug_mode = False):
    ...
    hypo_assignments = []   # dictionary list of propositions that are unassigned by can_props
    possible_conflicts = []
    prop_idx = 0
    for prop in prop_list:    # use the list for iteration to ensure consistent indexes!
        ...
        for can_prop in can_props:
            if can_prop.name == prop.name:
                ...
                if can_name_count > 1:
                    print('Warning: multiple candidate propositions with the same name')
    ...
    if log_test_result:
        ...
        if not debug_mode:
            ...
            return (conflicts, can_props, log_test_result, clause_logic_list, hypo_assignments)     
    
    # (4) if log_test_result is False, then we need to assess if any components are unassigned by cand_props, if they are then we can return hypothetical assignments and call this function again
    elif any(unassigned_list):
        #print('executing recursion')

        new_cand = find_highest_probability_proposition(hypo_assignments)
        # new_cand.support = clause   # This should already be accounted for by the Clause class __init__ method
        can_props.add(new_cand)
        # Recursively call the function until clause is satisfied or have conflicts
        return logical_test(clause, can_props, False)
    # (5) if log_test result is False and every element in unassigned_list is False, then we have identified conflicts
        ...
        for can_prop in can_props:
            if can_prop.support != 'Given':    # can_prop.support is a Clause class variable or 'Given'
...
def return_diagnoses(conflicts):
    ...
    # (3) Identify maximum value mode not present in conflicts for each component
        diagnoses.add(component.get_max_proposition(component_mode_dict[component])) 
    return diagnoses
...
best_diagnosis = find_highest_probability_diagnosis_set(output_kernels)
